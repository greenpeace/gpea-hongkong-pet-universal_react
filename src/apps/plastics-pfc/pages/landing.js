...
import { ChakraProvider, Box, Button, Divider, Image, Flex, Text, Heading, Link, SimpleGrid, Stack, Center, useColorModeValue, Circle, Icon, Grid, GridItem} from "@chakra-ui/react";
import SEO from "../SEO";
...
import DaluImage from "../assets/images/Screenshot01.png";
import Ann from "../assets/images/Ann.png";
import AnnImage from "../assets/images/Screenshot02.png";
import Kaman from "../assets/images/Kaman.png";
import Leanne from "../assets/images/Leanne.png";
import LeanneImage from "../assets/images/Screenshot03.png";
  ...
  const authorContent = [
    {
      ...
      title: '大埔街坊暨社區營造行動', 
      image: DaluImage,
      imageDescription: '連桷璋不時開設社區回收站「不是垃圾站 X 不是修理站」，推動資源共享分類回收。',
      content: '大埔街坊暨社區營造行動者，前大埔區議員，關注議題包括大埔公共空間發展、環保生態、社區經濟圈等。致力為街坊提供個案跟進、組織社區互助活動如社區回收站「不是垃圾站 X 不是修理站」、廚餘回收計劃及文化活動如中秋節大型月亮裝置藝術、政策倡議等。'
    },
    {name: 'Ann', nameZH: '李秀平', avatar: Ann, title: '「日常豊作」裸買店店主', image: AnnImage, imageDescription: '「日常豐作」裸買店店主Ann逢星期三也會開設有機菜攤檔。', content: '裸買店「日常豊作」店主，主張「祼買生活．無塑可能」環保健康豐足的生活哲學、鄰里互助與文化教育，建立裸買店聯盟，聚集一群有裸買店夢想的人，一同學習及分享開店心得。'},
    {name: 'Leanne', nameZH: '譚穎琳', avatar: Leanne, title: '綠色和平項目主任', image: LeanneImage, imageDescription: '綠色和平的「全城走塑計劃」集齊一眾走塑隊員在社區搜索走塑小店，自2018年至今，有164位參與，以及全港逾750間商戶加入走塑行列。看到「全城走塑」貼紙，即是找到走塑友善或提供走塑優惠的店舖!。', content: '全城走塑項目主任，倡議及推動走塑的無限可能，積極關心企業塑膠問題及香港塑膠政策。過去活躍於社區回收活動，看到源頭減廢的重要性，更加堅定了走塑的目標。希望廣招戰友一同打拼，一齊學一齊試，一齊達到無塑社區的願景!'},
    {name: 'Kaman', nameZH: '柯家文', avatar: Kaman, title: '綠色和平社區外展主任', image: null, content: '全城走塑社區外展主任，與「走塑小隊」捐窿捐罅，發掘社區走塑小店。疫情下，講環保，搞走塑，看似天方夜譚，但是我們「走塑小隊」發現，社區仍有好多街坊及小店堅持走塑。我們相信，落手落腳，邊玩邊學，今天一小步，社區一大步。成為我地走塑小隊一員啦，預埋你啦！！'},
  ]
        ...
        <Box flex="1" style={{ minWidth: "0px" }}>
          ...
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 140"><path fill="#F9BC81" d="M0,96L48,85.3C96,75,192,53,288,48C384,43,480,53,576,53.3C672,53,768,43,864,42.7C960,43,1056,53,1152,58.7C1248,64,1344,64,1392,64L1440,64L1440,0L1392,0C1344,0,1248,0,1152,0C1056,0,960,0,864,0C768,0,672,0,576,0C480,0,384,0,288,0C192,0,96,0,48,0L0,0Z"></path></svg>
            ...
            <Flex direction={{ base: "column", sm: "row" }}>
                <Box flex="1">
                  <Heading fontSize={{base: 'lg', sm: '2xl'}}>覺得自己只是被動的一方？見到社會好多事都看似沒可能改變？</Heading>
                  ...
                  <Grid templateColumns="repeat(5, 1fr)" gap={2} pt={4}>
                    <GridItem colSpan={{base: 5, sm: 2}}>
                        <Text color="#005F89" fontSize={'4xl'} fontSize={{base: '3xl', sm: '4xl'}} fontWeight={700} bgGradient="linear(to-l, #FF725C,#F9BC81)" bgClip="text" textAlign={{base: 'center', sm: 'right'}}>有你，有可能！</Text>
                    </GridItem>
                    <GridItem colSpan={{base: 5, sm: 3}} align="left" w="100%">
                      <Text color="#FF725C" fontSize={{base: '3xl', sm: '4xl'}} fontWeight={700} bgGradient="linear(to-l, #FF725C,#F9BC81)" bgClip="text" textAlign={{base: 'center', sm: 'left'}}>集齊腦袋，集思廣益！</Text>
                    </GridItem>
                  </Grid>
                  <Text as="p" {...pStyle}>
              ...
              <Box pt={{base: 10, sm: 20}}>
                <Flex direction="column">
                  ...
                  <Box bgColor="#FFF" py={8} px={{base: 2, sm: 8}} borderTop="1px solid #F9BC81">
                    <Box py={{base: 4, sm: 12}}>

                    <Flex direction={{base: 'column', sm: 'row'}} justify="center" align="center">
                    <Box flex="1" pr={{base: 0, sm: 12}}>
                      <Flex direction={{base: 'column', sm: 'row'}}>
                      <Heading fontSize={'2xl'} mb={4}>{current.title}&nbsp;</Heading>
                      <Heading fontSize={'2xl'} mb={4}><Text color="#005F89"> {current.nameZH}</Text></Heading>
                      </Flex>
                      <Text as="p" {...pStyle}>
                        {current.content}
                      </Text>
                      {current.imageDescription && <Text {...pStyle} color="brand.500">{current.imageDescription}</Text>}
                    </Box>
                    {current.image && <Box maxW="320px" pt={{base: 6, sm: 0}}>
