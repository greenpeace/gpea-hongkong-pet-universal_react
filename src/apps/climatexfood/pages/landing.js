...
const Landing = ({ submitted, togglePanel }) => {
  ...
  const Speaker = ({image, name, title, content, other}) => <Box mt={6} py={4}>
  <SimpleGrid columns={{ base: 1 }} p={6} borderRadius={"md"} bordertype='solid' borderWidth='1px' borderColor='brand.400'>
    <Box flex='1'>
      <Stack
              ...
              <Box flex='1'>
                ...
                <Flex direction={{base: 'column', sm: 'row'}}>
                  <Box flex={1} pr={{base: 0, sm: 3}}>
                    <Image borderRadius='8px' marginTop='8' src={info01} />
                    <Text {...subStyle}>綠色和平走到深水埗鬧市，展示巨型溫度計，呼籲港人正視氣候變化。</Text>
                  </Box>
                  <Box flex={1} pl={{base: 0, sm: 3}}>
                    <Image borderRadius='8px' marginTop='8' src={info02} />
                    <Text {...subStyle}>經過綠色和平長達3年的推動，荷蘭法院終裁定化石能源企業SHELL須為氣候危機負責，他們必須在9年內將其碳排放量，由2019年的水平大幅減少45％。</Text>
                  </Box>
                </Flex>
